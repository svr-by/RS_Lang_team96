...
export default class Statistic {
  ...
  async render() {
    let storage: IStatistic;
    const statNonUser = storageService.getSession('StatisticStorage') as IStatistic;
    if (statNonUser) {
      storage = statNonUser;
    } else {
    ...
    //   let userData = await api.getUserStatistics(userID, userToken);
    ...
    //       },
    //     };
    //     await api.saveUserStatistics(userID, userToken, storage);
    //     userData = (await api.getUserStatistics(userID, userToken)) as IStatistic;
    //   }
    //   const sumNewWords =
